...
import os
VS=float(os.environ.get('VS','0.5')); DZ=1.0
...
def _split_fast(v, f):
    """Connected components WITHOUT building a trimesh object.

    trimesh.split() on the multi-million-face fused mesh was the entire cost of
    v2: 372.9s of a 386s Duxton run, and 8.4GB / swap-thrash on Kent Ridge (which
    had to be killed). The expense is Trimesh(process=True) + split()'s cached
    adjacency, not the labelling itself. Labelling via the VERTEX graph is O(F),
    allocates one sparse matrix, and keeps no caches."""
    import scipy.sparse as _sp
    from scipy.sparse.csgraph import connected_components as _cc
    v = np.asarray(v); f = np.asarray(f)
    nv = len(v)
    e = np.vstack([f[:, [0, 1]], f[:, [1, 2]], f[:, [2, 0]]])
    g = _sp.coo_matrix((np.ones(len(e), np.int8), (e[:, 0], e[:, 1])), shape=(nv, nv))
    ncomp, vlab = _cc(g, directed=False)
    del e, g
    if ncomp == 1:
        return [(v, f)]
    flab = vlab[f[:, 0]]
    ...
    for c in range(ncomp):
        ...
        out.append((v[used].copy(), remap[fc]))
    return out
...
def main(B):
    ...
    for gi,g_ in enumerate(uq_g):
        ...
        try:
            ...
            chunks=_split_fast(rv,rf) if SPLIT_COMP else [(rv,rf)]
            for cv,cf in chunks:
                mm=trimesh.Trimesh(cv,cf,process=False); mm.merge_vertices()
                _one(mm, items, dtm)
            _nsolid+=len(chunks)
            del chunks
            continue
