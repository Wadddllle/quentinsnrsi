...
def shell_rings(shell, zc, tol=1e-6):
    ...
    for e in ol.entities:
        ...
        if len(pts)<4:
            REJ["short"]+=1; continue
        if abs(float(np.median(pts[:,2]))-zc)>0.25:
            REJ["offplane"]+=1; continue
        q=Polygon(pts[:,:2])
        if not q.is_valid:
            REJ["invalid"]+=1; REJ_AREA.append(("invalid",float(q.buffer(0).area))); continue
        if q.area<=0.05:
            REJ["tiny"]+=1; REJ_AREA.append(("tiny",float(q.area))); continue
        out.append(q)
    return out
...
def main(B):
    ...
    log(f"[REJECTS] {dict(REJ)}")
    _inv=sorted([a for k,a in REJ_AREA if k=="invalid"],reverse=True)
    _tin=sorted([a for k,a in REJ_AREA if k=="tiny"],reverse=True)
    log(f"[REJECTS] invalid n={len(_inv)} top_areas={[round(x,1) for x in _inv[:8]]}")
    log(f"[REJECTS] tiny    n={len(_tin)} top_areas={[round(x,4) for x in _tin[:8]]}")
    t=time.time()
