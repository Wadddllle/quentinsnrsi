...
def AUDIT(tag,v,f,cut=None):
    v=np.asarray(v,float); f=np.asarray(f)
    if len(f)==0: print(f"AUDIT {tag}: empty",flush=True); return
    ...
    bnd=ued[cnt==1]; at=0
    if cut is not None and len(bnd):
        zb=uq[bnd][:,:,2]; at=int((np.abs(zb-cut)<0.05).all(axis=1).sum())
    he=set(); wind=0
    for a_,b_,c_ in wf:
    ...
    print(f"AUDIT {tag:32s} f={len(wf):>8,} bnd={int((cnt==1).sum())-at:>6,} "
          f"NM={int((cnt>2).sum()):>5,} bow={bow:>4,} wind={wind:>5,} selfX={sx:>6,}",flush=True)
...
def main(B):
    ...
    for _i,_it in enumerate(items):
        _s=_it["shell"]
        pass  # AUDIT(f"7 shell#{_i} post-union/cap", _s.vertices, _s.faces, cut=_it["pad"])
        if _i>=4: break
    _av=[];_af=[];_n=0
    for _it in items:
        _av.append(np.asarray(_it["shell"].vertices)); _af.append(np.asarray(_it["shell"].faces)+_n)
        _n+=len(_it["shell"].vertices)
    pass  # AUDIT("7b, np.vstack(_av), np.vstack(_af))
    t=time.time()
    ...
    _tm.Trimesh(allv,allf,process=False).export(SP+"PROV_assembly.ply",encoding="binary")
    np.savez(SP+"PROV_assembly.npz", v=allv, f=allf)
    print("SAVED ply+npz", flush=True)
    ...
    trimesh.Trimesh(allv,allf,process=False).export(SP+"SLICECDT2_duxton.stl")
