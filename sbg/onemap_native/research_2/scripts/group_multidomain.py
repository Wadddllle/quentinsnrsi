...
def main(B):
    ...
    for gi,g_ in enumerate(uq_g):
        ...
        try:
            rv,rf=remesh(gv,gf)          # ONE level set for the whole group
            m=trimesh.Trimesh(rv,rf,process=True); m.merge_vertices()
            # NO SHRINK: the level set already merged touching members into one
            # solid, which is the entire point of grouping.
            ...
            nfail+=1
    log(f"[group-remesh] {len(items)} group solids, {nfail} failed  {time.time()-t:.1f}s "
    ...
    def FULLAUDIT(v,f,tag):
        ...
        for _vt,es in link.items():
            ...
            if comps>1: bow+=1
        ml=mn.meshFromFacesVerts(np.asarray(wf,np.int32),np.asarray(uq,float))
        pr=mr.findSelfCollidingTriangles(mr.MeshPart(ml)); nsx=pr.size()
        pen="n/a"
        if nsx:
            tri=uq[wf]; fa=[];fb=[]
            for i in range(nsx):
                q_=pr[i]; fa.append(int(q_.aFace)); fb.append(int(q_.bFace))
            A=tri[np.array(fa)]; Bq=tri[np.array(fb)]
            def pl(T):
                nn=np.cross(T[:,1]-T[:,0],T[:,2]-T[:,0])
                L=np.linalg.norm(nn,axis=1,keepdims=True); L[L==0]=1
                return nn/L
            dB=np.einsum('ijk,ik->ij',Bq-A[:,0][:,None,:],pl(A))
            dA=np.einsum('ijk,ik->ij',A-Bq[:,0][:,None,:],pl(Bq))
            pn=np.maximum(np.abs(dB).max(axis=1),np.abs(dA).max(axis=1))
            pen=f"med {np.median(pn):.3f}m p90 {np.percentile(pn,90):.3f}m max {pn.max():.3f}m"
        print(f"RESULT {tag} | faces={len(wf):,} | closed={'YES' if (cnt==1).sum()==0 else 'NO('+str(int((cnt==1).sum()))+')'}"
              f" | NM={int((cnt>2).sum())} | bowtie={bow} | wind={wind} | selfX={nsx} | pen={pen}",flush=True)
