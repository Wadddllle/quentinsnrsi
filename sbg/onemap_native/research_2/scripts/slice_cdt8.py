...
def _split_self_touching(pts):
    """Split a self-touching XY loop into SIMPLE loops at its repeated vertices,
    preserving EXACT coordinates.

    Why not buffer(0): it repairs validity by REWRITING geometry, so the ring no
    longer matches the shell rim it came from. Measured consequence: 124 shell rim
    vertices with no exact terrain counterpart (max 5.658m away) -> 134 open edges
    that capping cannot close, because the terrain hole simply isn't where the rim
    is. Splitting keeps every coordinate bit-identical, so the seam stays exact."""
    idx = {}; stack = []; loops = []
    for pt in pts:
        k = (round(float(pt[0]), 6), round(float(pt[1]), 6))
        if k in idx:
            j = idx[k]
            sub = stack[j:]
            if len(sub) >= 3: loops.append(np.array(sub))
            for q in stack[j:]:
                idx.pop((round(float(q[0]), 6), round(float(q[1]), 6)), None)
            stack = stack[:j]
        stack.append(pt); idx[k] = len(stack) - 1
    if len(stack) >= 3: loops.append(np.array(stack))
    return loops
...
def shell_rings(shell, zc, tol=1e-6):
    ...
    for e in ol.entities:
        ...
        if q.is_valid:
            ...
            # self-touching (pinched / figure-8) slice profile -> split at the
            # repeated vertices instead of buffer(0), to keep coordinates exact.
            cands=[]
            for sub in _split_self_touching(pts):
                sq=Polygon(sub[:,:2])
                if sq.is_valid: cands.append(sq)
        for part in cands:
